...
| §7.5 (D39–D41): residual correlation over long histories, panels instead of weights, the commit-reveal beacon | D39 done (T56, 2026-09-25): `collusion::{ResidualHistory, coordination_clusters}` reproduce Table 16 on the dataset ported to Rust (cartel +0.89 on residuals, honest +0.02; every cartel pair flagged, no honest pair) with `ρ_min = 0.7` (0.5 flags honest pairs by chance). D40 done (T57): `review::assign_diverse` seats at most one member of a cluster per panel, the extra round included, weights untouched — the uniform draw's 7.0% of panels with two or more members (Table 17) measured at 6.6% on the population ported to Rust, the diversified draw's 0%. D41 is T37 | `docs/01` D39, `docs/02` §Anti-collusion, `docs/08` COLLUSION-002/003/005/006 |
