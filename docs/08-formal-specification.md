...
#### BRIDGE-010 — The sides are the camps, the scores on the scale, and both sides rated the item
- **Claim (D42).** A few reviewers far out on the axis do not form a side of their own: the sides are the exact 1-D 2-means cut of `f_u`, each holding at least 5% of the axis reviewers. The side means average predictions clipped to [0, 1], so every score lies in [0, 1]. An item that fewer than `MIN_COVERAGE = 1` reviewers of either side rated is not decided by its score: the gate sends it to supplementary review and the re-decision cannot pass it.
- **Failure found (T24's first pass, 2026-09-26).** The iteration from the extremes of `f_u` formed sides of 2 and 5 of 800 reviewers (scores from −0.28 to 1.31) and, in a bootstrap subsample, of 1 of 200, dropping an item's robust score from 0.91 to 0.56 while its full fit passed; a partisan item with no minority rating passed at 0.972 on an extrapolated minority side of 1.445.
- **Evidence.** `side_split.rs` (AT-BR-11): two camps with two or five reviewers at four times the camp distance split into the camps; each side holds the floor on 256 random position sets with outliers; the split is invariant under the axis' gauge; clipped predictions keep the score in [0, 1]. `side_coverage.rs` (AT-BR-12): on the fixture plus an item only the camp of 120 rated at about 0.97, the gate returns `SupplementaryReview` (robust 0.875 passed before), the fixture's items keep their verdicts, and the re-decision passes it only once the other side has rated it, high. `side_evidence.rs` re-runs the three failures from their seeds: the partisan item goes to review, the scores stay in [0, 1], and the robust score at 60 boosters is 0.887 against a full 0.903 (the curve is monotone). `level_a.rs` still finds the fixture's sides of 80 and 120, and the oracle moved only in the side means of the three polarized items (≤ 0.0046, from the clipping).
- **Evidence status.** RESOLVED (D42, T71, 2026-09-26). The floor's value and `MIN_COVERAGE` are provisional (T25).
...
#### PROTO-010 — Governance — `stratified_sortition` TESTED; `change_approved` = 2/3 ∧ ≥ 30 days. The sortition draws from candidates carrying `f_u`, i.e. judge nyms; how a drawn judge nym then *acts* (produces golden items, sets blueprints) without linking to its propose nym is unspecified (PRIV-004). The sortition reads its candidates as a set (T72): ordered by position then id, it fills a deficit and returns its committee in that order — before, it filled and returned in the input order, so the same candidates reversed drew another committee even without ties (`draw_order.rs`, AT-BR-12).
...
| `Gated` | epoch scoring: `bridge_scores` → `bridging_gate(S_j, gap_j, coverage_j, τ, ε, γ_appeal)` (D32/T49: the robust side-balanced score and the side gap; D42/T71: an item below `MIN_COVERAGE` goes to supplementary review) | ratings of the whole epoch available; engine run reproducibly | `Pilot1` (Pass) / `SupplementaryReview` / `AppealEligible` / `Rejected` | scores published with checkpoint | scoring on a partial epoch |
...
| AT-BR-02 | crossing curve | boosters 0..80 in steps of 5, ≥ 50 random selections each — T24 study `bridging-capture` (`docs/13`: 200 drawn orders per cell, run pending) | crossing distribution reported with CI; docs updated | BRIDGE-005 |
...
| AT-BR-11 ✓ | outlying reviewers (D42) | two camps plus two or five reviewers far out on the axis; random position sets with outliers; predictions off the rating scale | the sides are the camps; each side holds at least 5% of the axis reviewers; the split does not depend on the axis' gauge; every score in [0, 1] — `side_split.rs`, and T24's failing replicates re-run in `side_evidence.rs` (T71) | BRIDGE-010 |
| AT-BR-12 ✓ | candidate order (T72) | the same candidates — 20 newcomers tied at the origin among 51 — listed in five orders; the sortition with ties and without | the same panel, extra panel and committee from the same seed — `draw_order.rs`: first panel and diversified panel over 100 seeds, both extra rounds over 30 beacons, the sortition over 100 seeds | PROTO-003, INV-10 |
| AT-BR-12 ✓ | an item one side never rated (D42) | the fixture plus an item only one camp rated at about 0.97 | the gate sends it to supplementary review (it passed at 0.875 before); the re-decision cannot pass it until the other side rates it — `side_coverage.rs`, and T24's partisan pass re-run in `side_evidence.rs` (T71) | BRIDGE-010 |
| AT-DIF-01 ◐ | FP rate | `n_biased = 0`, NT ∈ {1500, 3000}, K ∈ {4, 8, 16}, ≥ 200 seeds — T24 study `dif-null` (`docs/13`: NT up to 6,000, three anchor counts, run pending) | FP per item ≤ documented α — on the target model (T54, `latent_target_model.rs`): 0 of 120 clean items flagged and no batch with a mixture over 15 null batches — 20, 40 and 60 anchors (KR-20 0.79–0.94), four seeds at N = 3,000 and one at N = 12,000; the full design (≥ 200 seeds, K ∈ {4, 16}) is T24/T25 | DIF-008 |
| AT-DIF-02 | power surface | `δ ∈ {0.3, 0.5, 0.7, 0.9}`, `n_biased ∈ {1,2,3}`, `π ∈ {0.5, 0.3, 0.1}` — T24 study `dif-power` (`docs/13`: × NT ∈ {1500, 3000, 6000}, K ∈ {4, 8, 16}, run pending) | sensitivity table with CI; docs' "1500/3000" replaced by the table | STAT-001 |
| AT-DIF-03 | non-uniform DIF | class-specific `a_j` — T24 study `dif-nonuniform` (`docs/13`, run pending) | detected or documented as out of scope | DIF-004 |
| AT-DIF-04 | two axes | biased items split across two independent hidden axes — T24 study `dif-two-axes` (`docs/13`, run pending) | detected or documented | DIF-004 |
...
| AT-DIF-07 | sample poisoning | `c` coordinated respondents (c/NT ∈ {1,2,5,10 %}) answering to mask a real DIF / to create DIF on a clean item — T24 study `dif-poisoning` (`docs/13`, run pending) | fraction needed reported | §11.4 |
| AT-DIF-08 | purification oscillation | adversarial batch constructed so flags alternate | non-convergence signalled | DIF-007 |
| AT-DIF-09 | pool-scale mixture | K = 100, NT = 3000 — T24 study `dif-pool-scale` (`docs/13`, run pending) | completes within a stated budget | DIF-009 |
...
| BRIDGE-002 | axis recovery | `level_a.rs`, sim | TESTED (1 seed) | seed/parameter sweeps | `docs/13` `bridging-sweep` (run pending) |
| BRIDGE-003 | polarized items rejected | `level_a.rs` | TESTED (1 dataset) | calibration procedure for τ, λ | `docs/13` `bridging-sweep` (run pending), then T25 (`docs/07` §13) |
| BRIDGE-004 | bootstrap-min pessimistic & stable | `level_a.rs` (tautological) | IMPLEMENTED | warm vs cold comparison | new test |
| BRIDGE-005 | capture cost ≈ 87 % | `level_a.rs` (monotone only), sim | TESTED (qualitative) | crossing distribution | AT-BR-02 (`docs/13` `bridging-capture`, run pending); fix docs/06 figure |
...
| STAT-001 | 300/1500/3000 adequate | `power.rs` (ignored, 5 seeds) | HYPOTHESIS | power study | AT-DIF-02 (`docs/13` `dif-power`, run pending) |
...
| PROTO-010 | governance | `lifecycle.rs`, proptest, `draw_order.rs` | TESTED (mechanics); the sortition a function of the candidate set (T72) | acting-role linkage | G-19 |
